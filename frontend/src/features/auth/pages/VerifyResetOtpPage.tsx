...
function VerifyResetOtpPage() {
    ...
    const onSubmit = async (data: VerifyResetOtpFormData) => {
        ...
        try {
            ...
            if (axios.isAxiosError(e)) {
                setServerError(
                    e.response?.data?.message || "Ssomething went wrong"
                );
            }
        } finally {
            setLoading(false);
    ...
    return (
        <main className="flex min-h-screen w-full bg-white">
            ...
            <section className="relative hidden w-1/2 lg:block">
                <img
                    src="https://images.unsplash.com/photo-1695692929091-cdafc96ec082?q=80&w=1935&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                    alt="City skyline"
                ...
                <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="min-h-5">
                            <p className={`text-sm transition-colors ${serverError ? "text-red-600" : "text-transparent select-none"}`}>
                                {serverError || "\u00A0"}
                            </p>
                        </div>
                        ...
                        <p className="text-center text-sm text-slate-500">
                            ...
                            {canResend ? (
                                <button
                                    ...
                                    disabled={resending}
                                    className="font-medium text-blue-600 hover:text-blue-700 disabled:cursor-not-allowed disabled:text-blue-400"
                                >
